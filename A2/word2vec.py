...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_ = softmax(np.matmul(outsideVectors, centerWordVec))
    loss = - np.log(softmax_[outsideWordIdx])
    softmax_[outsideWordIdx] = softmax_[outsideWordIdx] - 1
    gradCenterVec = np.matmul(outsideVectors.T, softmax_)
    gradOutsideVecs = np.outer(softmax_, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    loss_term1 = - np.log(sigmoid(np.matmul(outsideVectors[outsideWordIdx], centerWordVec)))
    loss_term2 = 0
    for i in range(K):
        loss_term2 += - np.log(sigmoid(- np.matmul(outsideVectors[negSampleWordIndices[i]], centerWordVec)))
    ...
    sigmoid_ = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss -= np.log(sigmoid_)
    gradCenterVec += (sigmoid_ - 1) * outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx] += (sigmoid_ - 1) * centerWordVec

    for i in range(K):

        sig = sigmoid(centerWordVec.dot(-1*outsideVectors[indices[i+1]].T))

        loss -= np.log(sig)
        gradCenterVec += (1 - sig)*outsideVectors[indices[i+1]]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        loss_, gradCenter_, gradOutside_= \
        word2vecLossAndGradient(centerWordVec=centerWordVectors[word2Ind[currentCenterWord]],
            outsideWordIdx=word2Ind[word],outsideVectors=outsideVectors,dataset=dataset)
        loss += loss_
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenter_
