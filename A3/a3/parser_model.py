...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.Tensor(w.shape[0], self.embed_size * self.n_features)
        for idx, i in enumerate(w):
            x[idx] = self.embeddings[i].view(self.embed_size * self.n_features)
    ...
    def forward(self, w):
        ...
        before_relu = torch.matmul(x,self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        h = torch.relu(before_relu)
